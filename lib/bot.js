...
class Bot extends EventEmitter {
    ...
    sendFloatRequest(link) {
        return new Promise((resolve, reject) => {
            ...
            const proxyTag = String(
            (this.settings?.steam_user?.httpProxy) ||
            (this.settings?.steam_user?.socksProxy) ||
                'none'
            ).replace(/:\/\/.*@/, '://***@');
            this.currentPair = `${this.username || 'bot?'}|${proxyTag}`;
            console.log('[request start]', this.currentPair);
            ...
            this.ttlTimeout = setTimeout(() => {
                ...
                this.busy = false;
                this.currentRequest = false;
                console.log('[request done]', this.currentPair, this.currentRequestId);
                reject('ttl exceeded');
